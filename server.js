...
const { isGlobalSellingAuthorization,productQuestionSearchEndpoint,
  productQuestionAnswerEndpoint,productQuestionDetailEndpoint,communicationMessageDateValue,communicationMessageTimestamp,
...
async function initOrderManagementTables() {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS fulfillment_submissions (
    ...
    provider_order_number VARCHAR(120),
  ...
  await pool.query('ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS provider_order_number VARCHAR(120)');
  await pool.query('ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS remote_status VARCHAR(30)');
  await pool.query('ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS remote_returned BOOLEAN NOT NULL DEFAULT FALSE');
  await pool.query('ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS remote_message TEXT');
  await pool.query('ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS remote_checked_at TIMESTAMPTZ');
  await pool.query('ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS returned_at TIMESTAMPTZ');
  await pool.query('ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS warehouse_fee NUMERIC(12,2) NOT NULL DEFAULT 0');
  await pool.query('ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS service_fee NUMERIC(12,2) NOT NULL DEFAULT 0');
  ...
  await pool.query("ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS fulfillment_mode VARCHAR(20) NOT NULL DEFAULT 'express'");
  await pool.query("ALTER TABLE fulfillment_submissions ADD COLUMN IF NOT EXISTS stock_allocations JSONB NOT NULL DEFAULT '[]'::jsonb");
  ...
  if (payoutRepairSetting.rows[0]?.value !== payoutRepairVersion) {
    ...
    for (const row of cancelledRows) {
      const parsed = await normalizeParsedOrderBilling(
...
app.get('/api/health/order-management', (req, res) => {
  res.json({ code: 0, data: {
    ...
    orderLabelOfficialEndpoint: 'marketplace/shipments/{shipmentId}/labels',
    ...
    returnedFulfillmentRestoresWorkbench: true,
    ...
    yeekeIdentityFields: ['note','erpOrdersn','erpOrderSn','shopID','sysUserNote'],
    yeekeIdentityBadgeField: 'sysUserNote',
    ...
    lockedMixedCurrencyPayoutInvariant: '5.31-0.77-6.49+4.99=3.04',
    normalizedOfficialBillingLedgerCurrency: true,
    targetedMixedCurrencyPayoutRepair: true,
    ...
    mercadoLibreCurrentGlobalEndpoints: true,
    multiStoreCommunicationAggregation: true,
    communicationMessageObjectDates: true,
    communicationMissedFeedRecovery: true,
...
async function syncOrdersForUser(authUser, body = {}) {
  ...
  try {
    ...
    const strictSingle = Boolean(req.body?.strictSingle && requestedOrderId);
    ...
    // 单笔同步必须只处理该订单，不能因 pack/search 回退混入其他订单。
    if (strictSingle) {
      const allowedOrderIds = new Set(directOrderIds.map(String));
      sourceOrders = (sourceOrders || []).filter(order => allowedOrderIds.has(String(order?.id || '')));
    ...
    // 待发货截止时间优先采用 Mercado Libre 官方 shipment lead_time。
    ...
    try {
      const marketplaceSellerIds = await getOrderMarketplaceSellerIds(authUser.username,sellerId);
      const claimResponses = await Promise.all(marketplaceSellerIds.map(localSellerId =>
        axios.get(marketplaceClaimSearchEndpoint(), {
          params: { user_id: localSellerId, player_role: 'respondent', limit: 100 },
          headers: { Authorization: `Bearer ${accessToken}` }, timeout: 20000
        }).catch(() => null)));
    ...
    for (const order of sourceOrders) {
      ...
      const officialFinance = await normalizeParsedOrderBilling(
        parseOrderBilling(billingDetail,grossAmount),orderCurrency,getBillingFxRate
      );
      ...
      if (!order._shipment_lead_time_fetch_succeeded &&
          previous.rows[0]?.handling_deadline && previous.rows[0]?.deadline_is_estimated !== true &&
          (deadlineIsEstimated || !handlingDeadline)) {
...
app.post('/api/admin/orders/:orderId/sync', requireOrderAccess, async (req, res) => {
  const orderId = String(req.params.orderId || '').trim();
  const storeId = String(req.body?.storeId || '').trim();
  if (!orderId || !storeId) {
    return res.status(400).json({ code: 400, message: 'orderId and storeId are required' });
  }
  try {
    const data = await syncOrdersForUser(req.authUser,{ storeId,orderId,limit:1,strictSingle:true });
    res.json({ code: 0, data });
  } catch (e) {
...
async function getOrderListData(authUser,query = {}) {
  ...
  if (packedRows.length) {
    ...
    const submissionRows = await pool.query(`SELECT DISTINCT ON (f.order_id) f.id,f.order_id,f.warehouse_id,f.carrier,f.tracking_number,f.shipping_quantity,f.shipping_remark,f.fulfillment_mode,f.stock_allocations,f.status,
    ...
    const submissionMap = new Map(submissionRows.rows.map(row => {
      ...
      return [String(row.order_id),{
        ...
        fulfillmentMode:row.fulfillment_mode || 'express',stockAllocations:normalizeStockAllocations(row.stock_allocations),
...
async function getOrderInquiriesData(authUser,query = {}) {
    if (!String(query.storeId || '').trim()) {
      const authorizations=await listOrderStoreAuthorizations(authUser);
      if (authorizations.length>1) {
        const settled=await Promise.allSettled(authorizations.map(authorization=>getOrderInquiriesData(authUser,{
          ...query,storeId:String(authorization.ml_user_id)
        })));
        ...
        const mergeBy=(rows,keyOf)=>[...new Map(results.flatMap(result=>result[rows] || [])
          .map(item=>[keyOf(item),item])).values()];
        const productQuestionOrders=mergeBy('productQuestionOrders',item=>String(item.questionId || item.orderId || ''));
        const orderMessageOrders=mergeBy('orderMessageOrders',item=>String(item.packId || item.orderId || ''));
        const productQuestionItems=mergeBy('productQuestionItems',item=>String(item.id || item.question_id || item.questionId || ''));
        const orderMessageItems=mergeBy('orderMessageItems',item=>String(item.id || `${item.pack_id || item.order_id}:${communicationMessageDateValue(item)}:${item.text || item.message || ''}`));
        return {
          count:productQuestionOrders.length+orderMessageOrders.length,
          productQuestionCount:productQuestionOrders.length,
          orderMessageCount:orderMessageOrders.length,
          items:[...productQuestionItems,...orderMessageItems],
          productQuestionItems,orderMessageItems,
          orders:[...productQuestionOrders,...orderMessageOrders],
          productQuestionOrders,orderMessageOrders,
          newAlerts:results.reduce((sum,result)=>sum+Number(result.newAlerts || 0),0),
          questionError:results.map(result=>result.questionError).filter(Boolean).join('；')
        };
      }
    }
    const req={ authUser,query };
    ...
    if (req.query.background) {
      ...
      for (let index=0;index<threads.length;index+=5) {
        await Promise.all(threads.slice(index,index+5).map(async order=>{
          ...
          if (!packId) return;
          try {
            const response=await axios.get(orderPackMessagesEndpoint(context.authorization,packId,sellerId),{
              params:orderPackMessagesParams(context.authorization,{ markAsRead:false }),
      ...
      const batch = await Promise.all(recentResult.rows.slice(i, i + 5).map(async order => {
        ...
        try {
          ...
          const buyerMessages = messages.filter(message => {
            ...
            const timestamp = communicationMessageTimestamp(message);
            return !Number.isFinite(timestamp) || timestamp >= todayStart.getTime();
          });
    ...
    const itemMap = new Map();
    for (const item of [...todayItems, ...conversationItems]) itemMap.set(String(item.id || `${item.pack_id || item.order_id}:${communicationMessageDateValue(item)}:${item.text || item.message || ''}`), item);
...
async function getOrderAfterSalesData(authUser,query = {}) {
    if (!String(query.storeId || '').trim()) {
      const authorizations=await listOrderStoreAuthorizations(authUser);
      if (authorizations.length>1) {
        const settled=await Promise.allSettled(authorizations.map(authorization=>getOrderAfterSalesData(authUser,{
          ...query,storeId:String(authorization.ml_user_id)
        })));
        const results=settled.filter(result=>result.status==='fulfilled').map(result=>result.value);
        if (!results.length) throw settled[0].reason;
        const items=[...new Map(results.flatMap(result=>result.items || [])
          .map(item=>[String(item.id || `${item.storeId}:${claimOrderReference(item)}:${item.last_updated || ''}`),item])).values()];
        return {
          count:items.length,
          items,
          orders:[...new Map(items.map(item=>item.order).filter(Boolean)
            .map(order=>[String(order.orderId || order.packId),order])).values()],
          newAlerts:results.reduce((sum,result)=>sum+Number(result.newAlerts || 0),0)
        };
      }
    }
    ...
    const { token, sellerId } = context;
    const marketplaceSellerIds = await getOrderMarketplaceSellerIds(req.authUser.username,sellerId);
    const claimResponses = await Promise.all(marketplaceSellerIds.map(localSellerId => axios.get(marketplaceClaimSearchEndpoint(), {
      params: { status: 'opened', user_id: localSellerId, player_role: 'respondent' }, headers: { Authorization: `Bearer ${token}` }, timeout: 20000
    }).catch(error => ({ error }))));
    ...
    const closedClaimResponses=req.query.background ? await Promise.all(marketplaceSellerIds.map(localSellerId=>axios.get(
      marketplaceClaimSearchEndpoint(),{
      params:{ status:'closed',user_id:localSellerId,player_role:'respondent',limit:50 },
        headers:{ Authorization:`Bearer ${token}` },timeout:20000
      }).catch(()=>({ error:true })))) : [];
...
function decodeOfficialLabelError(error) {
  ...
  if (!receiverDocumentationPending && notPrintable && Object.keys(shipment).length) {
    ...
    const senderIds = officialShipmentSenderIds(shipment);
    const callerId = String(labelContext.actualCallerId || labelContext.sellerId || notPrintable[2] || '');
    const detail = [`官方实时状态 ${shipmentStatus}${shipmentSubstatus ? `/${shipmentSubstatus}` : ''}`];
    if (logisticType) detail.push(`物流类型 ${logisticType}`);
    if (senderIds.length) detail.push(`运单发件方 ${senderIds.join('/')}`);
    detail.push(`本次调用方 ${callerId}`);
    if (shipmentStatus !== 'ready_to_ship') {
      message = `运单 ${notPrintable[1]} 尚未达到官方可打印状态（${detail.join('，')}）。请先同步订单状态，待官方状态变为 ready_to_ship 后再申请面单。`;
    } else if (logisticType === 'fulfillment') {
      message = `运单 ${notPrintable[1]} 是 Fulfillment 运单，官方公共 API 不提供面单下载（${detail.join('，')}）。`;
    } else if (senderIds.length && !senderIds.includes(callerId)) {
      message = `运单 ${notPrintable[1]} 的官方发件方与当前授权不一致（${detail.join('，')}）。请授权运单发件方对应店铺后重试。`;
    } else {
      message = `运单 ${notPrintable[1]} 虽为待发货，但美客多仍未向该店铺开放 API 面单（${detail.join('，')}）。这不是串店调用；请在美客多卖家后台确认该运单面单是否已经生成。`;
    }
  }
  ...
  for (const request of officialLabelRequests(shipmentId)) {
    try {
      const response = await axios.get(request.url, {
        ...
        headers: { Authorization: `Bearer ${accessToken}`, Accept: 'application/pdf', 'x-format-new': 'true' },
...
async function resolveOfficialLabelStoreContext(authUser,orderRows,shipmentId) {
  ...
  const sortedCandidates = orderLabelAuthorizationCandidates(orderRows,authorizations,shipmentId);
  const candidates = sortedCandidates.filter(candidate => expectedStoreIds.includes(String(candidate.ml_user_id || '')));
  const available = [];
  const probeErrors = [];
  const queuedStoreIds = new Set(candidates.map(candidate => String(candidate.ml_user_id || '')));
  for (let candidateIndex = 0; candidateIndex < candidates.length; candidateIndex += 1) {
    const authorization = candidates[candidateIndex];
    let token;
    ...
    const context = {
      ...
      actualCallerId:String(authorization.ml_user_id || ''),
      verifiedOrderOwnership:false,
      verifiedShipmentAccess:false,
      expectedStoreIds,
      probedStoreUserIds:[]
    };
    ...
    try {
      ...
      context.actualCallerId = String(account.data?.id || context.sellerId);
    } catch (error) {
      probeErrors.push({ storeUserId:context.sellerId,probe:'users_me',status:error.response?.status || 0 });
    }
    try {
      const shipmentResponse = await axios.get(`https://api.mercadolibre.com/marketplace/shipments/${encodeURIComponent(shipmentId)}`, {
        headers:{ Authorization:`Bearer ${token}`,'x-format-new':'true' },timeout:15000
      });
      context.verifiedShipmentAccess = String(shipmentResponse.data?.id || '') === String(shipmentId);
      context.officialShipment = shipmentResponse.data || {};
      context.shipmentSenderIds = officialShipmentSenderIds(context.officialShipment);
      for (const senderId of context.shipmentSenderIds) {
        if (queuedStoreIds.has(senderId)) continue;
        const senderAuthorization = sortedCandidates.find(candidate => String(candidate.ml_user_id || '') === senderId);
        if (senderAuthorization) {
          candidates.push(senderAuthorization);
          queuedStoreIds.add(senderId);
        }
      }
    } catch (error) {
      probeErrors.push({ storeUserId:context.sellerId,probe:'shipment',status:error.response?.status || 0 });
    }
    for (const orderId of orderIds) {
      try {
        const orderPath = String(authorization.site_id || '').toUpperCase() === 'CBT'
          ? `marketplace/orders/${encodeURIComponent(orderId)}`
          : `orders/${encodeURIComponent(orderId)}`;
        const response = await axios.get(`https://api.mercadolibre.com/${orderPath}`, {
          headers:{ Authorization:`Bearer ${token}` },timeout:15000
        });
  ...
  const selectedContext = selectOrderLabelContext(available,expectedStoreIds);
  if (selectedContext) {
    selectedContext.probedStoreUserIds = available.map(item => item.actualCallerId || item.sellerId);
    selectedContext.probeErrors = probeErrors;
...
app.get('/api/admin/orders/:orderId/label', requireOrderAccess, async (req, res) => {
  ...
  try {
    ...
    const resolvedCallerId = String(context.actualCallerId || context.sellerId || '');
    audit.resolvedStoreUserId = resolvedCallerId;
    audit.probedStoreUserIds = context.probedStoreUserIds;
    audit.verifiedOrderOwnership = context.verifiedOrderOwnership;
    audit.verifiedShipmentAccess = context.verifiedShipmentAccess;
    audit.shipmentSenderIds = context.shipmentSenderIds || [];
    audit.officialShipment = context.officialShipment || {};
    let pdfResponse;
    ...
    for (const request of labelRequests) {
      try {
        pdfResponse = await axios.get(request.url, {
          params: request.params,
          headers: { Authorization: `Bearer ${context.token}`, Accept: 'application/pdf', 'x-format-new': 'true' },
          responseType: 'arraybuffer', timeout: 30000
        });
        if (pdfResponse?.data) break;
      } catch (error) {
        officialErrors.push({ error, sellerId: resolvedCallerId, url: request.url });
        console.warn('[Orders] 订单所属店铺面单请求失败:', resolvedCallerId, request.url, error.response?.status || error.message);
      }
    }
    if (!pdfResponse?.data) {
      ...
      if (preferredError) {
        audit.attemptedStoreUserIds = [resolvedCallerId];
        preferredError.error.labelContext = context;
    ...
    audit.storeUserId = targetRow.store_user_id;
    audit.successfulCallerId = resolvedCallerId;
    audit.attemptedStoreUserIds = [resolvedCallerId];
    await saveOfficialShippingLabel(req.authUser.username,targetShipmentId,req.params.orderId,targetRow.store_user_id,pdf);
    await saveOrderApiAudit(req.authUser.username,audit.storeUserId,req.params.orderId,'shipment_label',req.params.orderId,
      { success: true, shipmentIds, targetShipmentId, callerId: resolvedCallerId, verifiedOrderOwnership:context.verifiedOrderOwnership,
        verifiedShipmentAccess:context.verifiedShipmentAccess,shipmentSenderIds:context.shipmentSenderIds || [],
    ...
    await saveOrderApiAudit(req.authUser.username,audit.storeUserId,req.params.orderId,'shipment_label',req.params.orderId,
      { success: false, shipmentIds: audit.shipmentIds, targetShipmentId: audit.targetShipmentId, attemptedStoreUserIds: audit.attemptedStoreUserIds || [], attemptedCallerIds: audit.attemptedCallerIds || [], verifiedShipmentAccess:audit.verifiedShipmentAccess || false,shipmentSenderIds:audit.shipmentSenderIds || [],officialShipment:audit.officialShipment || {},status: official.status, officialError: official.auditData }).catch(() => {});
...
async function getOrderMessagesData(authUser,orderId) {
    ...
    if (normalizedOrderId.startsWith('question:')) {
      ...
      let raw=auditResult.rows[0].rawData || {};
      try {
        const response=await axios.get(productQuestionDetailEndpoint(context.authorization,questionId),{
          headers:{ Authorization:`Bearer ${context.token}` },timeout:20000
        });
        raw={ ...raw,...(response.data || {}) };
        await saveOrderApiAudit(authUser.username,auditResult.rows[0].storeId,normalizedOrderId,'presale_question',questionId,raw);
      } catch (error) {
        if (!raw || !Object.keys(raw).length) throw error;
        console.warn('[Orders] product question detail refresh failed, using audit cache:',authUser.username,questionId,error.response?.status || error.message);
      }
      const messages=[{
      ...
      const answer=raw.answer?.text ? raw.answer : raw._answer;
      if (answer?.text) messages.push({
        id:`answer:${questionId}`,from:{ user_id:auditResult.rows[0].storeId },
        message_date:answer.date_created || answer.answered_at || raw.last_updated,
        text:String(answer.text),message_type:'product_answer'
      });
    ...
    const response = await axios.get(orderPackMessagesEndpoint(context.authorization,packId,context.sellerId), {
      params:orderPackMessagesParams(context.authorization,{ markAsRead:true }),
      headers: { Authorization: `Bearer ${token}` }, timeout: 20000
    });
...
async function sendOrderMessageData(authUser,orderId,body = {}) {
    ...
    const { rows } = await pool.query('SELECT pack_id,store_user_id,buyer_id FROM ml_orders WHERE ml_order_id=$1 AND owner_username=$2', [String(orderId),authUser.username]);
    if (!rows[0]) { const error=new Error('订单不存在或无权操作'); error.status=404; throw error; }
    const context = await getOrderStoreContext(authUser,rows[0].store_user_id);
    if (!context) { const error=new Error('该订单所属店铺授权已失效'); error.status=403; throw error; }
    const token = context.token;
    const packId = rows[0].pack_id || String(orderId);
    const buyerId=String(rows[0].buyer_id || '').trim();
    if (!isGlobalSellingAuthorization(context.authorization) && !buyerId) {
      const error=new Error('订单缺少美客多买家身份，暂时无法通过官方接口回复');
      error.status=422;
      throw error;
    }
    const response = await axios.post(orderPackMessagesEndpoint(context.authorization,packId,context.sellerId),
      orderPackSendBody(context.authorization,{
        sellerId:context.sellerId,buyerId,text,textTranslated:body?.textTranslated
      }),
...
app.post('/api/admin/fulfillment-services', requireAdmin, async (req, res) => {
  ...
  const externalPrice = feeAmount(req.body?.externalPrice);
  const { rows } = await pool.query('INSERT INTO fulfillment_services(owner_username,name,code,description,external_price) VALUES($1,$2,$3,$4,$5) RETURNING id', [req.authUser.username,name.slice(0,120), String(req.body?.code || '').trim().slice(0,100), String(req.body?.description || '').trim().slice(0,500),externalPrice]);
...
async function getFulfillableWarehouseStock(ownerUsername, connector, { includeExhausted = false } = {}) {
  ...
  const usedResult = await pool.query(`SELECT stock_allocations FROM fulfillment_submissions
    WHERE owner_username=$1 AND warehouse_id=$2 AND fulfillment_mode='stock'
      AND status='success' AND remote_returned=FALSE`,[ownerUsername,connector.id]);
  const used = new Map();
  for (const row of usedResult.rows) {
    for (const allocation of normalizeStockAllocations(row.stock_allocations)) {
      used.set(allocation.remoteProductId,(used.get(allocation.remoteProductId) || 0) + allocation.quantity);
    }
  }
  ...
  const remoteRecords = extractWarehouseRecords(remote);
  const remoteMap = new Map(remoteRecords.map(item => [String(connector.provider === 'yeeke' ? (item.sysCode || item.id || '') : (item.stockPlusId || '')),item]));

  for (const local of localResult.rows) {
    if (remoteMap.has(String(local.remoteProductId))) continue;
    const targeted = connector.provider === 'yeeke'
      ? await client.listStock({ pageNo:1,pageSize:20,wareHouse:config.warehouseCode,sysCode:String(local.remoteProductId) })
      : await client.listStock({ pageNumber:1,pageSize:20,stockPlusDeliveryId:Number(config.storeAddressId),itemNo:String(local.sku),status:2,arrivedStore:1 });
    const match = extractWarehouseRecords(targeted).find(item => String(connector.provider === 'yeeke'
      ? (item.sysCode || item.id || '') : (item.stockPlusId || '')) === String(local.remoteProductId));
    if (match) remoteMap.set(String(local.remoteProductId),match);
  }
  ...
  return localResult.rows.map(local => {
    ...
    const localAvailable = Math.max(0,Number(local.receivedQuantity || 0) - allocatedQuantity);
    return {
  ...
  }).filter(item => includeExhausted || item.availableQuantity > 0);
}
...
app.get('/api/admin/warehouse-inventory/stock', requireOrderAccess, handleFulfillableWarehouseStock);
app.get('/api/admin/warehouse-inventory/fulfillable', requireOrderAccess, handleFulfillableWarehouseStock);
...
app.post('/api/admin/warehouse-inbounds', requireOrderAccess, async (req,res) => {
  ...
  try {
    ...
    if (connector.provider === 'yeeke') {
      ...
      const payload = buildYeekeInboundPayload({ ...requestSnapshot,items,warehouseCode:config.warehouseCode,
        localInboundNo:localNo,userIdentity:identity });
      ...
      for (const item of items) {
        ...
        const listed = extractWarehouseRecords(await client.listStock({ pageNumber:1,pageSize:20,
...
app.post('/api/admin/warehouse-inbounds/:id/sync', requireOrderAccess, async (req,res) => {
  ...
  if (!connector) return res.status(404).json({ code:404,message:'原仓库已停用，无法同步' });
    ...
    if (inbound.provider === 'yeeke') {
      ...
      const remoteItems = [];
      for (const item of inbound.items) {
        const remote = await client.listStock({ pageNumber:1,pageSize:20,stockPlusDeliveryId:Number(config.storeAddressId),itemNo:item.sku });
...
async function resolveOfficialLabelPdfForPush(authUser, orderRows) {
  ...
  for (const shipmentId of shipmentIds) {
    ...
    try {
      ...
      let labelResult;
      try {
        labelResult = await fetchOfficialLabelPdf(context.token,shipmentId);
      } catch (error) {
        error.labelContext = context;
        throw error;
      }
      const { pdf } = labelResult;
      await saveOfficialShippingLabel(authUser.username,shipmentId,String(order.ml_order_id || ''),order.store_user_id,pdf);
      return pdf;
    } catch (error) { lastError = error; }
  }
...
async function sendOrderToConnector(connector, row, options = {}, yeekeClient = null) {
  if (String(connector.provider || 'generic') === 'yeeke') {
    ...
    const payload = buildYeekeOrderPayload({
      ...
      stockAllocations: options.stockAllocations
    });
    try {
...
let yeekeSubmissionStatusSyncTimer = null;
let yeekeSubmissionStatusSyncRunning = false;
let yeekeSubmissionStatusSyncState = { lastRunAt:null,checked:0,returned:0,lastError:null };

async function syncYeekeSubmissionStatuses(ownerUsername = null,{ limit = 50,force = false } = {}) {
  if (yeekeSubmissionStatusSyncRunning && !ownerUsername) return { ...yeekeSubmissionStatusSyncState,running:true };
  if (!ownerUsername) yeekeSubmissionStatusSyncRunning = true;
  let checked = 0,returned = 0,lastError = null;
  try {
    const params = [];
    let ownerFilter = '';
    if (ownerUsername) {
      params.push(ownerUsername);
      ownerFilter = ` AND f.owner_username=$${params.length}`;
    }
    ...
    for (const submission of rows) {
      try {
        const connectorKey = String(submission.warehouse_id);
        let entry = clients.get(connectorKey);
        if (!entry) {
        ...
        if (!record) {
          ...
          [submission.id,submission.owner_username]);
          continue;
        }
        const remoteStatus = String(record.dgStatus ?? '').trim();
        ...
        if (isYeekeOrderReturned(record)) {
          ...
          if (updateResult.rowCount) {
            ...
            [submission.owner_username,submission.order_id,reason,
              `fulfillment-return:${submission.id}:${String(record.updateTime || remoteStatus || 'returned')}`]);
          }
        } else {
          await pool.query(`UPDATE fulfillment_submissions SET remote_status=$1,remote_message=$2,
            remote_checked_at=NOW(),updated_at=NOW() WHERE id=$3 AND owner_username=$4`,
          [remoteStatus || null,remoteMessage || null,submission.id,submission.owner_username]);
        }
      } catch (error) {
        lastError = String(error.response?.data?.message || error.message || error).slice(0,500);
        await pool.query('UPDATE fulfillment_submissions SET remote_checked_at=NOW() WHERE id=$1 AND owner_username=$2',
        [submission.id,submission.owner_username]).catch(() => {});
      }
    }
    const result = { lastRunAt:new Date().toISOString(),checked,returned,lastError };
    if (!ownerUsername) yeekeSubmissionStatusSyncState = result;
    return result;
  } finally {
    if (!ownerUsername) yeekeSubmissionStatusSyncRunning = false;
  }
}

function startYeekeSubmissionStatusSync() {
  if (yeekeSubmissionStatusSyncTimer) return;
  const tick = () => syncYeekeSubmissionStatuses(null,{ limit:50 }).catch(error => {
    yeekeSubmissionStatusSyncState = { ...yeekeSubmissionStatusSyncState,lastRunAt:new Date().toISOString(),lastError:String(error.message || error).slice(0,500) };
    console.error('[Yeeke] fulfillment status sync failed:',error.message);
  });
...
async function handleFulfillmentSubmit(req, res) {
  ...
  if (fulfillmentMode === 'express' && !carrier) return res.status(400).json({ code: 400, message: '请选择国内物流公司' });
  ...
  if (fulfillmentMode === 'stock' && !['yeeke','shopeex'].includes(provider)) {
    return res.status(400).json({ code:400,message:'库存发货仅支持 Yeeke 或 Shopeex/KJX 官方仓库' });
  }
  const carrierResult = fulfillmentMode === 'express' ? await pool.query(`SELECT lc.id,lc.code FROM logistics_companies lc
    JOIN users u ON u.username=lc.owner_username AND u.role='admin'
    WHERE lc.name=$1 AND lc.enabled=TRUE`,[carrier]) : { rows:[] };
  if (fulfillmentMode === 'express' && provider !== 'shopeex' && !carrierResult.rows[0]) return res.status(404).json({ code: 404, message: '请选择管理员维护的物流公司' });
  ...
  const requestedCarrierCode = fulfillmentMode === 'express' ? String(req.body?.carrierCode || '').trim() : '';
  const requestedCatalogCarrier = fulfillmentMode === 'express' && provider === 'shopeex' && requestedCarrierCode
    ? findShopeexLogisticsByCode(requestedCarrierCode)
    : null;
  if (fulfillmentMode === 'express' && provider === 'shopeex' && requestedCarrierCode && (!requestedCatalogCarrier || requestedCatalogCarrier.name !== carrier)) {
    return res.status(400).json({ code:400,message:'Shopeex/KJX 快递公司与快递代码不匹配，请重新选择' });
  }
  const carrierCode = fulfillmentMode === 'express' && provider === 'shopeex'
    ? (requestedCatalogCarrier?.code || resolveShopeexLogisticsCode(carrier,carrierResult.rows[0]?.code))
    : '';
  if (fulfillmentMode === 'express' && provider === 'shopeex' && !carrierCode) return res.status(400).json({ code:400,message:`国内物流公司“${carrier}”缺少 Shopeex/KJX 快递代码，请管理员在“仓库与增值服务 → 物流公司管理”中补充代码` });
  ...
  let fulfillableStockRows = [];
  if (fulfillmentMode === 'stock') {
    try {
      fulfillableStockRows = await getFulfillableWarehouseStock(req.authUser.username,warehouse);
    } catch (error) {
      return res.status(502).json({ code:502,message:`可发库存读取失败：${error.response?.data?.message || error.message}` });
    }
    if (!fulfillableStockRows.length) return res.status(409).json({ code:409,message:'当前仓库没有属于该用户且已经成功入库的可发库存' });
  }
  const results = [];
  for (const displayOrderId of orderIds) {
    ...
    try {
      stockAllocations = fulfillmentMode === 'stock'
        ? validateFulfillmentStockAllocations(orderResult.rows,stockByOrder[displayOrderId],fulfillableStockRows)
        : [];
    } catch (error) {
      results.push({ orderId:displayOrderId,success:false,message:error.message });
      continue;
    }
    const shippingQuantity = fulfillmentMode === 'stock'
      ? stockAllocations.reduce((total,item) => total + item.quantity,0)
      : Math.floor(Number(quantityByOrder[displayOrderId] ?? maximumShippingQuantity));
    ...
    const payload = { source:'shanyue-erp',action:isResubmit ? 'fulfillment_resubmit' : 'fulfillment_label',order_id:displayOrderId,
      ...
      fulfillment_mode:fulfillmentMode,stock_allocations:stockAllocations,
...
async function handleFulfillmentExpressUpdate(req, res) {
  ...
  if (submission.fulfillment_mode === 'stock') return res.status(400).json({ code:400,message:'库存发货没有国内快递号，不能使用快递号修改功能' });
...
app.get('/api/admin/fulfillment/submissions', requireOrderAccess, async (req, res) => {
  ...
  const { rows } = await pool.query(`SELECT f.id,f.order_id AS "orderId",f.warehouse_id AS "warehouseId",f.provider_order_number AS "providerOrderNumber",f.carrier,f.tracking_number AS "trackingNumber",
    f.shipping_quantity AS "shippingQuantity",f.shipping_remark AS "shippingRemark",f.fulfillment_mode AS "fulfillmentMode",f.stock_allocations AS "stockAllocations",f.status,
...
app.post('/api/admin/fulfillment/submissions/sync-status', requireOrderAccess, async (req, res) => {
  try {
    const result = await syncYeekeSubmissionStatuses(req.authUser.username,{ limit:100,force:true });
    res.json({ code:0,data:result,message:result.returned
      ? `已同步仓库状态，${result.returned} 个退回订单已恢复到订单工作台`
      : `已核对 ${result.checked} 个代贴单，暂未发现新的退回订单` });
  } catch (error) {
    res.status(502).json({ code:502,message:`同步仓库状态失败：${error.response?.data?.message || error.message}` });
  }
});

app.post('/api/admin/fulfillment/submissions/:id/retry', requireOrderAccess, async (req, res) => {
  const { rows } = await pool.query(`SELECT f.*,c.endpoint,c.auth_header,c.auth_value,c.provider,c.provider_config,c.unit_price FROM fulfillment_submissions f
    JOIN erp_connectors c ON c.id=f.warehouse_id
    JOIN users u ON u.username=c.owner_username AND u.role='admin'
  ...
  try {
    ...
    if (['yeeke','shopeex'].includes(String(submission.provider || 'generic'))) {
      ...
      const retryMode = submission.fulfillment_mode === 'stock' ? 'stock' : 'express';
      let retryStockAllocations = [];
      if (retryMode === 'stock') {
        const availableStock = await getFulfillableWarehouseStock(req.authUser.username,{ ...submission,id:submission.warehouse_id });
        retryStockAllocations = validateFulfillmentStockAllocations(orderResult.rows,submission.stock_allocations,availableStock);
      }
      let carrierCode = '';
      if (retryMode === 'express' && retryProvider === 'shopeex') {
      ...
      responseData = await sendOrderToConnector(submission, orderResult.rows, {
        ...
        fulfillmentMode: retryMode,
        stockAllocations: retryStockAllocations,
        pdfString,
        serviceCodes
      });
    } else {
...
app.put('/api/admin/erp-connectors/:id/price', requireAdmin, async (req, res) => {
  const unitPrice = feeAmount(req.body?.unitPrice);
  const { rowCount } = await pool.query('UPDATE erp_connectors SET unit_price=$1,updated_at=NOW() WHERE id=$2 AND owner_username=$3',
  [unitPrice,req.params.id,req.authUser.username]);
  if (!rowCount) return res.status(404).json({ code:404,message:'仓库不存在' });
  res.json({ code:0,data:{ id:Number(req.params.id),unitPrice },message:`仓库每单费用已设置为 CNY ${unitPrice.toFixed(2)}` });
});
...
let mercadoLibreMissedFeedTimer = null;
let mercadoLibreMissedFeedRunning = false;
let mercadoLibreMissedFeedState = { lastRunAt:null,received:0,accepted:0,lastError:null };
...
async function processMercadoLibreWebhookEvent(event,target) {
  ...
  if (event.topic === 'messages') {
    // Real-time callbacks normally contain a /marketplace/messages/... path,
    // while the official missed-feeds endpoint can return only the opaque
    // message id. In that case there is no valid resource URL to GET, so use
    // the official unread/messages endpoints to rebuild the local thread.
    const hasResourcePath=String(event.resource || '').startsWith('/');
    let message={};
    if (hasResourcePath) {
      try { message=await fetchWebhookResource(target,event); }
      catch (error) {
    ...
    const recovered=await getOrderInquiriesData(authUser,{ storeId:target.storeUserId,background:true });
    await touchOrderRealtimeState(pool,authUser.username,'communications',orderId || `message:${event.resourceId || ''}`);
    return { storeId:target.storeUserId,orderId,stored:true,newAlerts:recovered.newAlerts || 0 };
  }
  ...
  if (event.topic === 'claims') {
    ...
    const hasResourcePath=String(event.resource || '').startsWith('/');
    if (hasResourcePath) {
      try { claim=await fetchWebhookResource(target,event); }
      catch (error) {
        const status=Number(error.response?.status || error.status || 0);
        if (![403,404].includes(status)) throw error;
      }
    ...
    const recovered=await getOrderAfterSalesData(authUser,{ storeId:target.storeUserId,background:true });
    await touchOrderRealtimeState(pool,authUser.username,'communications',orderId || `claim:${event.resourceId || ''}`);
    return { storeId:target.storeUserId,orderId,stored:true,newAlerts:recovered.newAlerts || 0 };
  }
  return { ignored:true };
}
...
async function runMercadoLibreMissedFeedRecovery() {
  if (mercadoLibreMissedFeedRunning || !mercadoLibreWebhookService || !ML_CLIENT_ID) return;
  mercadoLibreMissedFeedRunning=true;
  let received=0,accepted=0,lastError=null;
  try {
    const { rows }=await pool.query(`SELECT a.*,COALESCE(u.role,'user') AS role
      FROM ml_store_authorizations a
      LEFT JOIN users u ON LOWER(u.username)=LOWER(a.owner_username)
      WHERE a.enabled=TRUE ORDER BY a.updated_at DESC LIMIT 5`);
    let response=null;
    for (const authorization of rows) {
      try {
        const token=await getStoreAuthorizationToken(authorization);
        if (!token) continue;
        response=await axios.get('https://api.mercadolibre.com/missed_feeds',{
    ...
    for (const notification of messages) {
      const result=await mercadoLibreWebhookService.enqueue(notification);
      if (result?.accepted) accepted++;
    }
  } finally {
    mercadoLibreMissedFeedRunning=false;
    mercadoLibreMissedFeedState={ lastRunAt:new Date().toISOString(),received,accepted,lastError };
  }
}

function startMercadoLibreMissedFeedRecovery() {
  if (mercadoLibreMissedFeedTimer) return;
  const tick=()=>runMercadoLibreMissedFeedRecovery().catch(error=>{
    mercadoLibreMissedFeedState={ ...mercadoLibreMissedFeedState,lastRunAt:new Date().toISOString(),lastError:String(error.message || error).slice(0,500) };
    console.error('[Mercado Libre] missed feeds recovery failed:',error.response?.data || error.message);
  });
  setTimeout(tick,15000).unref?.();
  mercadoLibreMissedFeedTimer=setInterval(tick,5*60*1000);
  mercadoLibreMissedFeedTimer.unref?.();
}

app.get('/api/health/mercadolibre-missed-feeds',(req,res) => res.json({ code:0,data:{
  enabled:Boolean(mercadoLibreMissedFeedTimer),running:mercadoLibreMissedFeedRunning,
  intervalSeconds:300,...mercadoLibreMissedFeedState
} }));
...
app.get('/api/health/yeeke-submission-status',(req,res) => res.json({ code:0,data:{
  enabled:Boolean(yeekeSubmissionStatusSyncTimer),running:yeekeSubmissionStatusSyncRunning,
  intervalSeconds:120,...yeekeSubmissionStatusSyncState
} }));
...
async function start() {
  ...
  startMercadoLibreMissedFeedRecovery();
  officialAccountService.start();
  startRealtimeOrderDiscovery();
  startYeekeSubmissionStatusSync();
  ...
  const shutdown = signal => {
    ...
    if (mercadoLibreMissedFeedTimer) clearInterval(mercadoLibreMissedFeedTimer);
    if (yeekeSubmissionStatusSyncTimer) clearInterval(yeekeSubmissionStatusSyncTimer);
    if (melTrackingBackfillTimer) clearTimeout(melTrackingBackfillTimer);
